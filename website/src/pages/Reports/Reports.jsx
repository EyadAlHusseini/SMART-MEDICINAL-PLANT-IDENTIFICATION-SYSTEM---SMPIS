...
/* --- THEME COLORS --- */
const PIE_COLORS = ["#062d23", "#2d7260", "#88c3a9", "#cfe4d9", "#a3b18a"];

export default function Reports() {
  const { records, theme } = useData();
  ...
  const chartAccentColor = isDarkMode ? "#10b981" : "#0a3f31"; // Brighter emerald for dark mode
  const gridColor = isDarkMode ? "rgba(255,255,255,0.05)" : "#e2e8f0";
  ...
  // 2. DYNAMIC LOGIC: Daily & Weekly Data
  const chartData = useMemo(() => {
    ...
    for (let i = 4; i >= 0; i--) {
      ...
      data.push({
        name: label,
        classifications: records.filter((r) => r.date === label).length,
      });
    }
    return data;
  }, [records]);

  // 3. DYNAMIC LOGIC: Species Stats
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={chartData}>
              ...
              <Tooltip
                ...
                cursor={{
                  fill: isDarkMode ? "rgba(255,255,255,0.05)" : "#f1f5f9",
                }}
              />
              <Bar
                dataKey="classifications"
      ...
      {/* ROW 3: Plant Distribution & Top Species (RESTORED) */}
          ...
          <div className="space-y-3">
            {plantStats.length > 0 ? (
              plantStats.slice(0, 5).map((plant, i) => (
                <div
                  key={plant.name}
                  className="flex items-center justify-between p-4 bg-emerald-50/40 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/20 rounded-xl hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all"
                >
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/5 rounded-xl p-8 shadow-sm transition-colors">
        <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-8">
      ...
      <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800/30 rounded-xl p-4 px-6 text-sm text-slate-600 dark:text-emerald-300 shadow-sm transition-colors">
        <strong className="text-emerald-800 dark:text-emerald-400 font-bold">
          Note:
        </strong>{" "}
        All charts and reports are print-ready. Use Export PDF for formal
        documentation or Export Excel for data analysis.
      </div>
    </div>
  );
}
...
function KPICard({ title, value, trend, isGreen }) {
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-xl p-5 min-h-[140px] flex flex-col justify-between transition-colors shadow-sm">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div
          className={`flex items-center gap-1.5 text-sm font-medium ${isGreen ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}
        >
          {isGreen && (
            <svg
              width="16"
...
function CustomTooltip({ active, payload, label }) {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 p-3 shadow-xl rounded-lg">
